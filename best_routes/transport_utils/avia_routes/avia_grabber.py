...
def get_avia_routes_from_service(service: AviaService, content: dict) -> list:
    ...
    if request_object["request"] is not None:
        future = _get_future(request_object)
...
def get_avia_trips_from_service(service: AviaService, content: dict) -> list:
    count = int(content["count"])
    content_to = _copy_dict(content)
    content_back = _copy_dict(content)
    _fill_contents(content_to, content_back, content)

    request_object_to = service.get_request_object(content_to)
    request_object_back = service.get_request_object(content_back)

    if request_object_to["request"] is not None and request_object_back["request"] is not None:
        future_to = _get_future(request_object_to)
        future_back = _get_future(request_object_back)
        ...
        return _make_trips(routes_to, routes_back)
    return []
...
def get_avia_trips(content: dict) -> list:
    ...
    jsoned_trips = _make_trips(routes_to_sorted, routes_back_sorted)
...
def _make_trips(routes_to: list, routes_back: list) -> list:
    result = []
...
def _copy_dict(base_dict: dict) -> dict:
    new_dict = {}
...
def _get_future(request_object: dict) -> FuturesSession:
...
def _fill_contents(content_to: dict, content_back: dict, content: dict) -> None:
